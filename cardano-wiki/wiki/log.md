...
## [2026-05-03] plan | proof-experiment harness + Goblin agentic framework — OpenSpec changes drafted, QA reviewed, Codex handoff ready
- OpenSpec change `add-proof-experiment-harness` at openspec/changes/add-proof-experiment-harness/. 7 new crates: omega-claim-tx, omega-claim-prover, omega-claim-verifier, omega-mock-ledger (rusqlite/SQLite + WAL + actor pattern), omega-toy-consensus (openraft 0.9), omega-network (libp2p 0.55), omega-experiment CLI. Validates strict.
- OpenSpec change `add-goblin-agentic-framework` at openspec/changes/add-goblin-agentic-framework/. 3 new crates: omega-api (axum REST + OpenAPI + WS), omega-goblin-core (LlmClient trait + Role + Action), omega-goblin-runner (parameterizable Gemma-4 E4B agents). 6 default roles: Holder, Whale, Adversary, Lurker, SnapshotServer, Validator. Validates strict.
- QA review at openspec/changes/add-proof-experiment-harness/QA-REVIEW.md flagged 3 P0 + 8 P1 + 3 P2 + 10 missed failure modes. All P0s addressed in artifacts: actor pattern explicit (not spawn_blocking-per-call), concrete version pins (libp2p 0.55, openraft 0.9, plonky3 git-rev), gossipsub dropped from v0.1 (Raft AppendEntries authoritative). Load-bearing P1s addressed: ≤64-byte leaf-preimage soundness boundary, snapshot wire protocol explicit, restart-durability test added, --tap-cardano slot-stall fallback.
...
## [2026-05-04] docs | rustdoc convention + workspace doc-pass over four soundness-bearing crates
- New local skill at `skills/local/omega-rustdoc-style/SKILL.md` codifies the rustdoc convention: every public item gets one-line summary; functions returning `Result` get `# Errors`; soundness-bearing items get `# Soundness` blocks naming the invariant + closed attack + failure mode; v0.1/v0.2 boundaries get `# Limitations` blocks; crate-level docs carry elevator pitch + design-context links + tier-of-trust.
- Doc pass dispatched as four parallel agents, one per crate. All four landed clean:
  - `omega-commitment-core` — full crate-level docstring + `Hash`/`blake3_256`/`sha3_256`/`dual_hash` (drift-detection-not-break-hedge framing) + `MerkleTree::build_v1` (3 named invariants) + `leaf_hash_v2`/`node_hash_v2` (domain-separation soundness blocks) + all 7 leaf encoders + `InclusionWitness::verify` (flagged real soundness gap: legacy raw `blake3(left||right)` rather than `node_hash_v2`; documented not fixed) + `serde_helpers` marked `#[doc(hidden)]`.
  - `omega-claim-tx` — full crate doc + `ClaimTx`/`ClaimUtxo`/`ClaimCollection`/`ClaimPublicInputs` (with `# Soundness` calling out v2-bump fields) + `ClaimWitness` (privacy-redaction note) + `to_cbor`/`from_cbor` `# Errors` enumerating every `CborError` + canonicality check.
  - `omega-claim-prover` — full crate doc + `OmegaMembershipAir` (9-item constraint contract by trace-column index) + `prove_collection` `# Errors` + `# Soundness` listing all public values + `OmegaCommitment` + `OmegaBlake3Air` (batched-STARK participant role) + `proof_airs` + `blake3_trace` + `prove_collection_with_trace_tamper` marked `#[doc(hidden)]`.
  - `omega-claim-verifier` — full crate doc per the SKILL example + `verify` with full `# Errors` (8 variants) + `# Soundness` (4 numbered conditions on the witness, LogUp gluing, binding-digest envelope-rewrite check) + 8-variant `VerifyError`.
- Workspace `Cargo.toml` gained `[workspace.lints.rust] missing_docs = "warn"` + `[workspace.lints.rustdoc] broken_intra_doc_links = "warn", missing_crate_level_docs = "warn"`. Per-crate `[lints.rust]` blocks collapsed to `[lints] workspace = true` in all four touched crates.
- Task 12.7 added to `add-proof-experiment-harness/tasks.md` mandating SKILL compliance for all future code.
- Verification gate (workspace): `cargo doc --workspace --no-deps --document-private-items` clean (no warnings), `cargo clippy --workspace --all-targets -- -D warnings` clean, `cargo test --workspace --no-fail-fast` 48 test groups, 0 failures.
- Pre-existing HTML-tag warning in `omega-utxo-snapshot/src/main.rs:55` fixed in passing.
